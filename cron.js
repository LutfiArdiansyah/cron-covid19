...
schedule.scheduleJob("*/60 * * * *", async function () {
  ...
  https.get(
    "https://data.covid19.go.id/public/api/data.json_=" + Date.now().toString(),
    function (response) {
      response.pipe(data);
    }
  );
  ...
  https.get(
    "https://data.covid19.go.id/public/api/update.json?_=" +
      Date.now().toString(),
    function (response) {
  ...
  https.get(
    "https://data.covid19.go.id/public/api/prov.json_=" + Date.now().toString(),
    function (response) {
      response.pipe(prov);
    }
  );
